...
class User {
  ...
  static async findAll() {
    ...
    return rows.map(row => new User(row.first_name, row.last_name, row.email, null, row.id, row.avatar));
  }

  static async findById(id) {
    const query = 'SELECT id, first_name, last_name, email, avatar FROM users WHERE id = $1';
    const values = [id];
    const { rows } = await pool.query(query, values);
    if (rows.length > 0) {
      return new User(rows[0].first_name, rows[0].last_name, rows[0].email, null, rows[0].id, rows[0].avatar);
    }
    return rows[0];
  }
